...
class Recycler:
    ...
    def move_to_buffer_bin(self, file_path: str):
        ...
        for path in destinies.keys():
            try:
                ...
                self.prompt.say(f"Moving {TCOLORS["path"]}{path}{TCOLORS["style end"]}"
                                f"to buffer bin: {TCOLORS["path"]}{
                                    destinies[path]["target"]}"
                                f"{TCOLORS["style end"]}")
                self.__db_log(
                    destinies[path]["name"],
                    destinies[path]["path"],
    ...
    def move_to_recycle_bin(self, file_path: str):
        ...
        for path in destinies.keys():
            ...
            try:
                ...
                print(f"{TCOLORS["error"]}Error moving "
                      f"{TCOLORS["path"]}{path}{TCOLORS["error"]}to recycle bin: {e}")
    ...
    def empty_buffer_bin(self):
        flag = self.prompt.verify(f"{TCOLORS["warning"]}Warning{TCOLORS["style end"]}"
                                  ": This action will move all files in the buffer bin"
                                  "to recycle bin and it is not reversible,"
                                  "are you sure(y/n):")
        if not flag:
            self.prompt.say("Buffer bin will not be emptied.")
        else:
            self.move_to_recycle_bin(self.buffer_bin_path)
            self.__db_clear()
